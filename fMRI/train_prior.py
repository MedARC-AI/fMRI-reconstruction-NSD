...
if __name__ == '__main__':
    ...
    print('diffusion_prior.voxel2clip', diffusion_prior.voxel2clip)
    ...
    for epoch in progress_bar:
        ...
        for train_i, (voxel, image) in enumerate(train_dl):
            ...
            with torch.cuda.amp.autocast():
                ...
                clip_voxels = diffusion_prior.voxel2clip(voxel.float())
                ...
                if clip_aug_mode == 'x':
                    ...
                    if random.random() < clip_aug_prob:
                        ...
                        loss, pred = diffusion_prior(text_embed=clip_aug, image_embed=clip_image, voxel=voxel)
                        loss_on_aug.append(loss.item())
                    else:
                        loss, pred = diffusion_prior(text_embed=clip_voxels, image_embed=clip_image, voxel=voxel)
                        loss_off_aug.append(loss.item())

                elif clip_aug_mode == 'y':
                    # the input x is fixed, and we will change the target y
                    if random.random() < clip_aug_prob:
                        _, clip_pred = diffusion_prior(text_embed=clip_voxels, image_embed=clip_image, voxel=voxel)
                        ...
                        loss, pred = diffusion_prior(text_embed=clip_voxels, image_embed=clip_aug, voxel=voxel)
                        loss_on_aug.append(loss.item())
                    else:
                        loss, pred = diffusion_prior(text_embed=clip_voxels, image_embed=clip_image, voxel=voxel)
                        loss_off_aug.append(loss.item())
        ...
        for val_i, (val_voxel, val_image) in enumerate(val_dl):    
            with torch.no_grad(): 
                ...
                val_voxel = voxel.to(device)

                with torch.cuda.amp.autocast():
                    clip_voxels = diffusion_prior.voxel2clip(val_voxel.float())
                    #clip_voxels = nn.functional.normalize(clip_voxels,dim=-1)
                    #clip_voxels = clip_extractor.embed_curated_annotations(subj01_annots[voxel])
                    clip_image = clip_extractor.embed_image(val_image).float()
                    val_loss, val_pred = diffusion_prior(text_embed=clip_voxels, image_embed=clip_image, voxel=val_voxel)
        ...
        logs = {
            ...
            "train/fwd_pct_correct": fwd_percent_correct / train_i,
            "train/bwd_pct_correct": fwd_percent_correct / train_i,
            "train/val_fwd_pct_correct": val_fwd_percent_correct / val_i,
            "train/val_bwd_pct_correct": val_fwd_percent_correct / val_i,
        }
