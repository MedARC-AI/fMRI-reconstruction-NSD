...
import utils
from models import Voxel2StableDiffusionModel, Voxel2StableDiffusionContModel, Voxel2StableDiffusionTinyModel, SelfDistillMLP
...
student_mlp = SelfDistillMLP()
sd = {k:v.clone() for k,v in student_mlp.state_dict().items()}
teacher_mlp = SelfDistillMLP()
teacher_mlp.load_state_dict(sd)
...
if voxel_dims == 1: # 1D data
    voxel2sd = Voxel2StableDiffusionModel(use_cont=False)
    # 134M params
...
student_mlp = DDP(student_mlp, device_ids=[local_rank])
...
opt_grouped_parameters = [
    ...
    {'params': [p for n, p in student_mlp.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 1e-2},
    {'params': [p for n, p in student_mlp.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0},
]
optimizer = torch.optim.AdamW(opt_grouped_parameters, lr=1e-3)
lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=[max_lr, max_lr, 1e-3, 1e-3], 
                                            total_steps=num_epochs*((num_train//batch_size)//num_devices), 
                                            final_div_factor=1000,
                                            last_epoch=-1, pct_start=2/num_epochs)
teacher_momentum = utils.cosine_anneal(0.996, 1.0, (num_train/(batch_size*torch.cuda.device_count()))*num_epochs)
    ...
    if local_rank==0:
        ...
        try:
            torch.save({
                ...
                'student_state_dict': student_mlp.module.state_dict(),
                'teacher_state_dict': teacher_mlp.state_dict(),
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, _) in enumerate(train_dl):
        ...
        with torch.cuda.amp.autocast(enabled=use_mp):
            ...
            image_enc_aug = autoenc.encode(2*image_512_aug-1).latent_dist.mode() * 0.18215
            ...
            student_proj = student_mlp(image_enc_pred)
            teacher_proj = teacher_mlp(image_enc)
            teacher_proj_aug = teacher_mlp(image_enc_aug)
            cont_loss = utils.soft_cont_loss(
                student_proj.reshape(-1, student_proj.shape[-1]), 
                teacher_proj.reshape(-1, student_proj.shape[-1]), 
                teacher_proj_aug.reshape(-1, student_proj.shape[-1])
            )
            ...
            if use_reconst:
                ...
                reconst = autoenc.decode(image_enc_pred[:4]/0.18215).sample
                reconst_loss = F.l1_loss(reconst, 2*image[reconst_select]-1)
            ...
            loss = mse_loss/0.18215 + 4*reconst_loss + cont_loss
        ...
        # # EMA update teacher
        params_q = [param_q for param_q in student_mlp.parameters()]
        params_k = [param_k for param_k in teacher_mlp.parameters()]
        with torch.no_grad():
            m = teacher_momentum[global_it]  # momentum parameter
