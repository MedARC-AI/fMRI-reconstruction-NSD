...
pretrained = False # use pretrained prior
dim = 768
depth = 6
dim_head = 64
...
cache_dir = "/fsx/proj-medarc/fmri/atom/wds-cache"
n_cache_recs = 0
# -----------------------------------------------------------------------------
...
def create_models_and_loaders(using_ddp, local_rank, device):
    global lr_scheduler  # for some reason this is needed
    if local_rank == 0:
    ...
    if distributed:
        diffusion_prior = DDP(diffusion_prior, device_ids=[local_rank], output_device=local_rank)
    try:
    ...
    return diffusion_prior, optimizer, clip_extractor, sd_pipe, train_dl, val_dl, lr_scheduler
...
def main():
    ...
    if distributed:
        using_ddp, local_rank, device = ddp_config.set_ddp()
    else:
        using_ddp, local_rank, device = False, 0, torch.device('cuda:0')
    ...
    diffusion_prior, optimizer, clip_extractor, sd_pipe, train_dl, val_dl, lr_scheduler = create_models_and_loaders(using_ddp, local_rank, device)
    ...
    if local_rank==0:
        ...
        progress_bar = range(epoch, num_epochs)
    ...
    for epoch in progress_bar:
        ...
        # validation on ddp not working
        if local_rank == -1:  # validate only on main_proc
...
if __name__ == "__main__":
    # To train without ddp: python train_prior_w_voxel2clip.py --distributed=False
    # To train with ddp: python -m torch.distributed.launch --nproc_per_node 8 train_prior_w_voxel2clip.py
    main()